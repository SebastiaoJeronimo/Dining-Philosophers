 ...
 * ASK DIOGO IF THE TIMMING IS 200 or 201 and if starts in 0 or 1
*/
void *routine_one()
{
    printf("%d %d %s",1 , 1, M_FORK);
    printf("%d %d %s",get_data()->time_die + 1 , 1, M_DIE);
    return 0;
}
...
void one_philo_case()
{
    ...
    //COMPLETE ONE PHILO AND TEST WITH VALGRIND
    if (!philo->fork)
        return (free(philo));
